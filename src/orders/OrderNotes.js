...
export default function OrderNotes () {
  ...
  if (error) { return <p>El pedido no tiene notas.</p>; }
  return (
    <>
      {data.map((note) => (
        <Box mt={1.8}>
          <Typography variant="body2"><div dangerouslySetInnerHTML={{__html:note.note}} /></Typography>
          <Typography sx={{ fontSize: 12 }} color="text.secondary">{new Date(note.date_created).toLocaleDateString('es-PE', dateSettings)}</Typography>
        </Box>
      ))}
    </>
  );
